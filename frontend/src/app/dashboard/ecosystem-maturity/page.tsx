          ...
          {ecosystem && (
            <div className="space-y-3">
              <div className="flex items-end gap-3">
                ...
                <span className="text-sm text-slate-400">active of {safeArr(ecosystem.integration_breakdown).length}</span>
              </div>
              <div className="text-sm text-slate-500">
                Health: <span className="text-slate-200">{(ecosystem.ecosystem_health_score * 100).toFixed(0)}%</span>
              </div>
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {safeArr<{ name: string; type: string; status: string; health: number }>(ecosystem.integration_breakdown).slice(0, 6).map((int, i) => (
              ...
              {safeArr<string>(ecosystem.optimization_opportunities).length > 0 && (
                <div className="pt-2 border-t border-slate-700/40">
                  <div className="text-xs text-slate-500 mb-1">Opportunities:</div>
                  {safeArr<string>(ecosystem.optimization_opportunities).map((o, i) => (
        ...
        <motion.div {...slideUp} className="bg-surface-card border border-surface-border rounded-xl p-6">
          ...
          {trace && (
            <div className="space-y-3">
              ...
              {safeArr<{ from_system: string; to_system: string; latency_ms: number; complete: boolean }>(trace.system_boundaries).map((b, i) => (
              ...
              {safeArr(trace.gap_analysis).length > 0 && (
        ...
        <motion.div {...slideUp} className="bg-surface-card border border-surface-border rounded-xl p-6">
          ...
          {compliance && (
            <div className="space-y-3">
              ...
              {safeArr<{ quarter: string; compliance_score: number; frameworks_applied: string[] }>(compliance.compliance_evolution).map((q, i) => (
              ...
              <div className="text-xs text-slate-500">
                Frameworks: {safeArr<{ quarter: string; compliance_score: number; frameworks_applied: string[] }>(compliance.compliance_evolution)[safeArr<{ quarter: string; compliance_score: number; frameworks_applied: string[] }>(compliance.compliance_evolution).length - 1]?.frameworks_applied.join(", ") || "N/A"}
              </div>
            </div>
          )}
        </motion.div>
