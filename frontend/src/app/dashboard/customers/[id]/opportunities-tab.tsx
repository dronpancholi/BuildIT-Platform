...
export function OpportunitiesTab({ customerId }: { customerId: string }) {
  ...
  const { data: keywords = [], isLoading: loadingKeywords } = useQuery<Keyword[]>({
    ...
    queryFn: async () => {
      ...
      return allKeywords.slice(0, 20); // Limit to 20 for performance
    },
  ...
  const opportunities: Opportunity[] = [
    ...
    ...keywords
      .filter((k) => k.search_volume > 1000 && k.difficulty < 40)
      .slice(0, 5)
      .map((k) => ({
        type: "keyword" as const,
        title: `Target "${k.keyword}"`,
        description: `High volume (${k.search_volume.toLocaleString()}) with low difficulty (${k.difficulty})`,
        priority: "high" as const,
        impact: k.search_volume * (100 - k.difficulty) / 100,
        data: k,
      })),
    
    // Commercial intent keywords
    ...keywords
      .filter((k) => k.intent === "commercial" && k.search_volume > 500)
      .slice(0, 5)
      .map((k) => ({
        type: "keyword" as const,
        title: `Optimize for "${k.keyword}"`,
        description: `Commercial intent keyword with ${k.search_volume} monthly searches`,
        priority: "medium" as const,
        impact: k.search_volume * 0.8,
        data: k,
      })),
  ];
  ...
  const stats = {
    ...
    highVolume: keywords.filter((k) => k.search_volume > 1000).length,
    commercialIntent: keywords.filter((k) => k.intent === "commercial").length,
    totalProspects: allProspects.length,
    highQualityProspects: allProspects.filter((p) => p.composite_score > 0.7).length,
                ...
                {opp.type === "keyword" && opp.data && (
                  <div className="flex items-center gap-4 text-[10px] font-mono text-slate-600">
                    <span className="flex items-center gap-1">
                      ...
                      Vol: {opp.data.search_volume.toLocaleString()}
                    </span>
                    <span className="flex items-center gap-1">
                      <TrendingUp className="w-3 h-3 text-amber-500" />
                      KD: {opp.data.difficulty}
                    </span>
                    <span className="flex items-center gap-1">
