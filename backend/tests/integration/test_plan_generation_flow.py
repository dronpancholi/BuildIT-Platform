...
@pytest.mark.asyncio
async def test_full_plan_flow(client, tenant_id, setup_db, monkeypatch):
    ...
    monkeypatch.setattr("seo_platform.services.plan_simulator.get_tenant_session", fake_ctx)
    monkeypatch.setattr("seo_platform.services.plan_optimizer.get_tenant_session", fake_ctx)
    # Mock memory service for planning (no historic entries).
    with patch("seo_platform.services.planning_engine.memory_service") as mock_mem:
        ...
        plan_data = gen_resp.json()["data"]
        plan_id = plan_data["id"]
        # 2. Simulate plan.
        sim_resp = await client.post(
            f"/api/v1/plans/{plan_id}/simulate",
            params={"tenant_id": str(tenant_id)},
        )
        assert sim_resp.status_code == 200
        sim_data = sim_resp.json()["data"]
        assert sim_data["status"] == "SIMULATED" or sim_data["status"] == "simulated"
        # 3. Optimize plan.
        opt_resp = await client.post(
            f"/api/v1/plans/{plan_id}/optimize",
            params={"tenant_id": str(tenant_id)},
        )
        assert opt_resp.status_code == 200
        opt_data = opt_resp.json()["data"]
        assert "risk_score" in opt_data
        assert "confidence_score" in opt_data
        # 4. Forecast.
        fc_resp = await client.get(
            f"/api/v1/plans/{plan_id}/forecast",
            params={"tenant_id": str(tenant_id)},
        )
        assert fc_resp.status_code == 200
        forecasts = fc_resp.json()["data"]
